...
cp .env.example .env            # then fill in OLLAMA_BASE_URL / OLLAMA_MODEL / OLLAMA_EMBED_MODEL / DATABASE_URL
```
...
Make sure the backend's `uvicorn app.main:app --reload` (run from `backend/`) is running first.

### How it works

`backend/app/graph.py` builds a single-node LangGraph graph with one tool,
`search_faq`, which does similarity search over the mock FAQ documents in
`backend/app/knowledge_base.py` (embedded in-memory via Ollama embeddings). The
